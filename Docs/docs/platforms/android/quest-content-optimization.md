---
title: "Android Content Optimization"
...
Here, we'll give you some general guidelines of what you need to keep in mind while building content for VRChat on Android.
...
The Android version of VRChat has several hard (and soft) limits for content on avatars. Check out [Android Content Limitations](/platforms/android/quest-content-limitations) to find out more, as well as our page [Avatar Performance Ranking System](/avatars/avatar-performance-ranking-system) for some more details on how blocking works.
...
Of course, the profiler on your powerful PC won't represent how a profiler on the Android might look, but you can still see that X component is using a ton of frame time versus rendering, or etc. Its all relative!
...
You cannot upload or access worlds that exceed 100MB in size after build-time compression for VRChat on Android.
...
Keeping triangle count low is very important on mobile platforms. Even the Quest, which is quite powerful for a mobile headset, does have hardware limits. Keeping an eye on your triangle count is very important to keep performance high.
...
While building worlds, you should try to keep triangle count low. You want to leave room for the user's avatars as well. **We recommend that you budget approximately 250,000 triangles for your world in total.**
...
Shaders are restricted for avatars in VRChat on Android, and you can only use the VRChat Mobile shaders included in the VRChat SDK.

You can read about these variants on our [Android Content Limitations](/platforms/android/quest-content-limitations) page.
...
Shaders are not restricted for worlds in VRChat on Android. However, you should be *extremely careful* when writing and using custom shaders. Aim for performance above all else. If you're looking for a highly optimized basic world shader, use `Mobile/VRChat/Lightmapped` and bake your lighting.
...
Cloth components are disabled completely in VRChat on Android.

### Cameras

Camera components are disabled on avatars in VRChat on Android.

They are permitted in worlds, but you should be careful not to go overboard with them.

### Lights

Lights are disabled completely on avatars in VRChat on Android.

### Post Processing (v1 and v2)

Post processing systems are disabled completely in VRChat on Android.

### Audio Sources

Audio sources are disabled completely on avatars in VRChat on Android.

Audio sources are restricted in worlds in VRChat on Android.

### Rigidbodies, Colliders, and Joints

Rigidbodies, colliders, and joints are disabled completely on avatars in VRChat on Android.

They are permitted in worlds, but you should be careful not to go overboard with them.

### Particles

Particles are limited heavily on avatars in VRChat on Android.
