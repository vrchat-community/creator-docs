...
The `VRCLookAtConstraint` component is a simplified [Aim Constraint](/common-components/constraints/vrc-aim-constraint). It rotates the target transform so its positive Z axis faces towards the sources.
